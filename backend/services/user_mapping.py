# -*- coding: utf-8 -*-
"""
Mapping giữa user và Họ và Tên nhân viên từ file user-ten.txt
"""
...
LOWER_USER_MAP = {k.strip().lower(): v for k, v in USER_TEN_MAP.items()}
# Reverse mapping: Họ tên (lower) -> List username
NAME_TO_USERS = {}
for u, n in LOWER_USER_MAP.items():
    nl = n.strip().lower()
    NAME_TO_USERS.setdefault(nl, []).append(u)


def get_user_full_name(user: str) -> str:
    """Trả về Họ và Tên tương ứng với username, fallback về user gốc nếu không có."""
    if not user:
        return ""
    key = str(user).strip().lower()
    return LOWER_USER_MAP.get(key, str(user))


def get_usernames_by_name(name: str):
    """Tìm các username khớp với Họ và tên (hoặc chính username)."""
    if not name:
        return []
    key = str(name).strip().lower()
    if key in LOWER_USER_MAP:
        return [key]
    if key in NAME_TO_USERS:
        return NAME_TO_USERS[key]
    return [name]
